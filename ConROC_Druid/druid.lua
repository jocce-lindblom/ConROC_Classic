...
--	elseif IsSpellKnown(ids.Bal_Ability.ImprovedFaerieFireRank1) then _FaerieFire = ids.Bal_Ability.ImprovedFaerieFireRank1; end
	...
	if IsSpellKnown(ids.Bal_Ability.WrathRank10) then _Wrath = ids.Bal_Ability.WrathRank10;
	...
	if IsSpellKnown(ids.Feral_Ability.LacerateRank4) then _Lacerate = ids.Feral_Ability.LacerateRankRank4;
	elseif IsSpellKnown(ids.Feral_Ability.LacerateRankRank3) then _Lacerate = ids.Feral_Ability.LacerateRankRank3;
	elseif IsSpellKnown(ids.Feral_Ability.LacerateRankRank2) then _Lacerate = ids.Feral_Ability.LacerateRankRank2; end
...
--Conditions
	...
	if plvl >= 10 and IsSpellKnown(Feral_Ability.GrowlRank1) then
		tarInMelee = ConROC:Targets(Feral_Ability.GrowlRank1);	
...
--Rotations	
	...
	if cFormBUFF then
		...
		if ripRDY and not ripDEBUFF and combo >= 4 and energy >= 30 and not (ConROC:CreatureType("Undead") or ConROC:CreatureType("Mechanical") or ConROC:CreatureType("Elemental")) then
			return _Rip;
	...
	if bFormBUFF then
		...
		if lacerateRDY and (not lacerateDEBUFF2 or lacerateCount < 5) and bMangleDUR >= 2 and rage >= 15 then
			return _Lacerate;
		end
	...
	if not (bFormBUFF or cFormBUFF) then
